...
const getStops = function(req, res, next) {
  if(!req.body.latLon) return next(new Error('no latLon on OBA request'));
  let obaSURL = 'http://api.pugetsound.onebusaway.org/api/where/stops-for-location.json?key=3b0b239f-bc24-45e1-ada2-3e5f31bb655a&lat=' + req.body.latLon.lat + '&lon=' + req.body.latLon.lng + '&radius=300&maxCount=10';
  ...
  http.get(obaSURL, (response, err) => {
    ...
    response.on('end', () => {
      ...
      let obaRURL = 'http://api.pugetsound.onebusaway.org/api/where/routes-for-location.json?key=3b0b239f-bc24-45e1-ada2-3e5f31bb655a&lat=' + req.body.latLon.lat + '&lon=' + req.body.latLon.lng + '&radius=300&maxCount=100000';
